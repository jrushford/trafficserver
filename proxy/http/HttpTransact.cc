...
HttpTransact::handle_cache_write_lock(State *s)
{
  ...
  case CACHE_WL_FAIL:
    ...
    case CACHE_WL_FAIL_ACTION_COLLAPSED_FORWARDING:
      // This uses its own cf_debug tag because this debug can be useful for metrics
      if (s->state_machine->get_cache_sm().get_open_write_tries() < s->txn_conf->max_cache_open_write_retries) {
        if (s->state_machine->get_cache_sm().get_open_write_tries() == 1) {
          TxnDebug("cf_debug", "collapsed forwarding, cache miss, retry read with collapsed forwarding try #1");
        }
        ...
        TxnDebug("cf_debug", "cache_open_write_fail_action %d, miss, ran out of write retries (%d), doing default",
