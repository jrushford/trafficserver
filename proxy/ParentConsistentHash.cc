  ...
  if (!pRec || (pRec && !pRec->available.load()) || host_stat == TS_HOST_STATUS_DOWN) {
    do {
      ...
      if (pRec && !pRec->available.load() && host_stat == TS_HOST_STATUS_UP) {
        Debug("parent_select", "Parent.failedAt = %u, retry = %u, xact_start = %u, retriers = %d, max_retriers = %d",
              static_cast<unsigned>(pRec->failedAt.load()), static_cast<unsigned>(retry_time),
              static_cast<unsigned>(request_info->xact_start), pRec->retriers(), max_retriers);
