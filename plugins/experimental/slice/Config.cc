...
Config::fromArgs(int const argc, char const *const argv[])
{
  ...
  constexpr struct option longopts[] = {
    ...
    {const_cast<char *>("effective-url"), no_argument, nullptr, 'e'},
    {nullptr, 0, nullptr, 0},
  };
  ...
  for (;;) {
    int const opt = getopt_long(argc + 1, argvp, "b:t:p:de", longopts, nullptr);
    if (-1 == opt) {
      break;
    }
    ...
    case 'e':
      m_urltype = EFFECTIVE;
      DEBUG_LOG("Switching URL type to EFFECTIVE");
      break;
    case 'd':
      m_paceerrsecs = -1;
      break;
