...
ParentRecord::ProcessParents(char *val, bool isPrimary)
{
  ...
  for (int i = 0; i < numTok; i++) {
    ...
    if (isPrimary) {
      ...
      HostStatRec_t *hst = hs.getHostStatus(this->parents[i].hostname);
      if (!hst || hst->status == HostStatus_t::HOST_STATUS_INIT) {
...
ParentRecord::Init(matcher_line *line_info)
{
  ...
  for (int i = 0; i < MATCHER_MAX_TOKENS; i++) {
    ...
    if (strcasecmp(label, "round_robin") == 0) {
      ...
      // ignore_self_detect defaults to false.
      if (strcasecmp(val, "true") == 0) {
...
#define FP                                                           \
  ...
  // HostStatus now loads persisted states from records.snap.
  // In order to avoid susbsequent regression test failures
  // due to tests marking these hosts down and the state was persisted,
  // begin by marking these hosts up and available for unit tests.
  ...
  _st.setHostStatus("larry", HOST_STATUS_UP, 0, &Reason::MANUAL);
...
#define TEST_IP6_ADDR "BEEF:DEAD:ABBA:CAFE:1337:1E1F:5EED:C0FF"
  ...
  FP;
  RE(verify(result, PARENT_SPECIFIED, "odie", 80) + verify(result, PARENT_SPECIFIED, "garfield", 80), 6);
  // Test 7
  ST(7);
  REINIT;
  ...
  FP;
  RE(verify(result, PARENT_SPECIFIED, "fuzzy", 80), 8);
  ...
  br(request, "i.am.rabbit.net");
  FP;
  RE(verify(result, PARENT_SPECIFIED, "fluffy", 80), 9);
  // Test 10
  ST(10);
  REINIT;
  br(request, "i.am.rabbit.net");
  FP;
  RE(verify(result, PARENT_SPECIFIED, "furry", 80), 10);
  // Test 11
  ST(11);
  REINIT;
  br(request, "i.am.rabbit.net");
  FP;
  RE(verify(result, PARENT_SPECIFIED, "frisky", 80), 11);
  // restart the loop
  // Test 12
  ST(12);
  REINIT;
  ...
  br(request, "i.am.rabbit.net");
  FP;
  RE(verify(result, PARENT_SPECIFIED, "fluffy", 80), 13);
  // Test 14
  ST(14);
  REINIT;
  br(request, "i.am.rabbit.net");
  FP;
  RE(verify(result, PARENT_SPECIFIED, "furry", 80), 14);
  // Test 15
  ST(15);
  REINIT;
  br(request, "i.am.rabbit.net");
  FP;
  RE(verify(result, PARENT_SPECIFIED, "frisky", 80), 15);
  params->markParentDown(result, fail_threshold, retry_time);
  ...
  br(request, "i.am.rabbit.net");
  FP;
  RE(verify(result, PARENT_SPECIFIED, "fluffy", 80), 17);
  // Test 18
  ST(18);
  REINIT;
  br(request, "i.am.rabbit.net");
  FP;
  RE(verify(result, PARENT_SPECIFIED, "furry", 80), 18);
  // Test 19
  ST(19);
  REINIT;
  br(request, "i.am.rabbit.net");
  ...
  br(request, "i.am.rabbit.net");
  FP;
  RE(verify(result, PARENT_SPECIFIED, "fluffy", 80), 21);
  // Test 22
  ST(22);
  REINIT;
  br(request, "i.am.rabbit.net");
  FP;
  RE(verify(result, PARENT_SPECIFIED, "furry", 80), 22);
  params->markParentDown(result, fail_threshold, retry_time);
