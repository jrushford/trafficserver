...
struct strip_state *
strip_state_new(size_t buffer_ct)
{
  struct strip_state *strp = TSmalloc(sizeof *strp);
  strp->strip_uri          = (char *)TSmalloc(buffer_ct);
  memset(strp->strip_uri, 0, buffer_ct);
  strp->strip_uri_ct = buffer_ct;

  return strp;
}

void
strip_state_delete(struct strip_state *strp)
{
  if (strp->strip_uri) {
    TSfree(strp->strip_uri);
  }
  TSfree(strp);
}

void
get_redirect_renew_url(struct strip_state *strp, const char *new_token, char *new_url, size_t new_url_ct)
{
  static const char *sub_delim_string = "!$&\'()*+,;=";

  /* new_url_ct must be of at least the size of stripped_uri + new_token + null termination + sub-delim or reserved*/
  if (new_url_ct < (strp->strip_uri_ct + strlen(new_token) + 2)) {
    PluginDebug("Buffer for new redirect transport url not large enough for new token.");
    return;
  }

  /* The following code could be made more effecient with the use of memcpy or strcpy.
   * This can be changed if looking to optimize */
  if (strp->term != 0 && strchr(sub_delim_string, strp->term)) {
    /* Replace from first char of package to terminating sub-delim */
    strncpy(new_url, strp->strip_uri, strp->index);
    new_url[strp->index] = '\0';
    strcat(new_url, new_token);
    int len          = strlen(new_url);
    new_url[len]     = strp->term;
    new_url[len + 1] = '\0';
    strcat(new_url, strp->strip_uri + strp->index);
  } else {
    /* Replace from preceding reserved char to last char of token */
    strncpy(new_url, strp->strip_uri, strp->index);
    new_url[strp->index]     = strp->reserved;
    new_url[strp->index + 1] = '\0';
    strcat(new_url, new_token);
    strcat(new_url, strp->strip_uri + strp->index);
  }
  return;
...
cjose_jws_t *
get_jws_from_uri(const char *uri, size_t uri_ct, const char *paramName, size_t buff_ct, struct strip_state *strp)
{
  ...
  for (;;) {
    ...
    if ((size_t)(key_end - key) == termination_ct && !strncmp(paramName, key, (size_t)(key_end - key))) {
      ...
      if (!jws) {
        ...
        strp->strip_uri_ct = ((key - uri) + (end - value_end));
        if (buff_ct <= strp->strip_uri_ct) {
        ...
        if (value_end != end && strchr(sub_delim_string, *value_end)) {
          ...
          strp->index = key - uri;
          memcpy(strp->strip_uri, uri, (key - uri));
          memcpy(strp->strip_uri + (key - uri), value_end + 1, (end - value_end + 1));
        } else {
          /*Strip from reserved char to the last char of the JWT */
          strp->index = key - uri - 1;
          memcpy(strp->strip_uri, uri, (key - uri - 1));
          memcpy(strp->strip_uri + (key - uri - 1), value_end, (end - value_end));
        }

        if (strp->strip_uri[strp->strip_uri_ct] != '\0') {
          strp->strip_uri[strp->strip_uri_ct] = '\0';
        }
        PluginDebug("Stripped URI: %s", strp->strip_uri);
      }
      return jws;
    }
  }
...
struct jwt *
validate_jws(cjose_jws_t *jws, struct config *cfg, const char *uri, size_t uri_ct, int *rc)
{
  struct timer t;
  int64_t last_mark = 0;
  start_timer(&t);
